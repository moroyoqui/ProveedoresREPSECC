...
- Q (local): ¿Cuáles periodos periódicos concretos se consideran "activos" y pueden generar estado "Faltante" para un proveedor? → A: **Todos los periodos del año natural en curso cuya `coverage_period_start ≤ today`**. Los periodos futuros (`coverage_period_start > today`) NO generan obligación "Faltante". Ejemplo: el 12-jun-2026 para documentos bimestrales, los periodos ene-feb, mar-abr y may-jun son activos; jul-ago aún no lo es.
- Q (local): Al cruzar el año nuevo (1 de enero), ¿qué ocurre con los periodos del año anterior que nunca tuvieron documento cargado? → A: Se archivan como **"Faltante histórico"**: siguen siendo visibles en el detalle del proveedor (sección colapsable por año o tab de historial) pero **no cuentan en el indicador de cumplimiento activo del año en curso**. El porcentaje de cumplimiento activo se calcula exclusivamente contra los periodos del año natural en curso.
- Q (local): Un proveedor dado de alta a mitad del año (p.ej. 1 de mayo 2026): ¿se le exigen los bimestres anteriores a su alta en ese mismo año? → A: **No**. Los periodos cuyo `coverage_period_start < Supplier.created_at` no generan obligación "Faltante". El primer periodo exigible es aquel cuyo inicio es mayor o igual a la fecha de alta del proveedor en el sistema.

### Session 2026-05-18

- Q (local): ¿Qué efecto tiene sobre los documentos ya cargados cambiar el `SupplierType` de un proveedor desde la pantalla de edición? → A: Si el proveedor tiene al menos un documento cuya **fecha de vencimiento efectiva** (`effective_due_date`) cae en el **año natural en curso**, el sistema exige una **confirmación destructiva** antes de aplicar el cambio: un popup modal advierte que esos documentos serán eliminados permanentemente, pide al usuario escribir la palabra `eliminar` en una caja de texto y habilita el botón de confirmación solo cuando el texto coincide. Al confirmar, el sistema elimina definitivamente esos documentos (registro + archivo físico), aplica el nuevo tipo de proveedor y recalcula los documentos requeridos y el estado de cumplimiento agregado, todo en una sola operación transaccional. Si el proveedor NO tiene documentos cuyo vencimiento caiga en el año en curso, el cambio se aplica directamente sin popup. Documentos con vencimiento en años distintos, sin vigencia (NULL) o versiones históricas archivadas con vencimiento fuera del año en curso NO se eliminan.
- Q (local): ¿La edición de los datos generales del proveedor (razón social, RFC, contacto, estado) requiere alguna confirmación especial? → A: No. La edición de campos generales se persiste con validación estándar (formato y unicidad de RFC dentro del tenant) y se registra en `audit_log`. Solo el cambio de `SupplierType` puede disparar el flujo destructivo descrito arriba.
...
1c. **Given** un administrador autenticado en la pantalla de edición de un proveedor existente, **When** modifica campos generales (razón social, RFC, contacto principal, estado activo/inactivo) y guarda, **Then** el sistema valida formato y unicidad de RFC dentro del tenant, persiste los cambios, registra el evento en `audit_log` y NO toca documentos cargados ni dispara ningún popup de confirmación destructiva.
1d. **Given** un proveedor con al menos un documento cuya `effective_due_date` cae en el año natural en curso, **When** el administrador cambia el `SupplierType` desde la edición y presiona guardar, **Then** el sistema abre un popup modal destructivo que muestra el conteo de documentos que serán eliminados, exige escribir exactamente la palabra `eliminar` (case-insensitive, sin espacios al inicio/final) en una caja de texto y mantiene el botón de confirmación deshabilitado hasta que el texto coincide.
1e. **Given** el popup destructivo abierto del escenario 1d con el texto `eliminar` válido, **When** el administrador presiona el botón de confirmación, **Then** el sistema, en una sola transacción, elimina permanentemente cada documento del proveedor cuya `effective_due_date` cae en el año en curso (registro + archivo físico), aplica el nuevo `SupplierType`, recalcula los documentos requeridos y el cumplimiento agregado, y registra en `audit_log` un evento por cada documento eliminado y un evento `supplier.type_changed`. Si cualquier paso falla, ningún efecto persiste.
1f. **Given** el popup destructivo abierto del escenario 1d, **When** el administrador presiona cancelar o cierra el popup, **Then** el `SupplierType` original se conserva, los documentos quedan intactos y no se registra ningún evento de cambio.
1g. **Given** un proveedor SIN documentos cuyo `effective_due_date` caiga en el año natural en curso, **When** el administrador cambia el `SupplierType` y guarda, **Then** el cambio se aplica directamente sin popup destructivo y los documentos requeridos se recalculan inmediatamente conforme a FR-012b.
...
- ¿Qué pasa si durante la confirmación destructiva del cambio de `SupplierType` el usuario cancela o cierra el popup? El cambio NO se aplica, los documentos del proveedor permanecen intactos y no se registra evento alguno en `audit_log`.
- ¿Qué pasa con documentos sin vigencia (NULL) o con vencimiento en años distintos al año en curso cuando se confirma el cambio de tipo? NO se eliminan. La acción destructiva solo borra documentos cuya `effective_due_date` cae estrictamente dentro del año natural en curso.
- ¿Qué pasa con versiones históricas archivadas de un documento que se elimina por cambio de tipo? Las versiones cuya `effective_due_date` también cae en el año en curso se eliminan; las versiones con vencimiento fuera del año en curso se conservan como histórico desligado del documento vigente, marcadas como huérfanas en `audit_log`.
- ¿Qué pasa si el nuevo `SupplierType` que el administrador eligió no exige ningún `DocumentType` que el proveedor ya tenía cargado fuera del año en curso? Esos documentos permanecen como histórico consultable pero dejan de contar como requisito activo; el indicador agregado se recalcula contra el nuevo conjunto de requisitos (FR-012b).
- ¿Qué pasa si la transacción del flujo destructivo (FR-005d) falla a mitad de la ejecución (p. ej. error al borrar archivo físico)? La transacción se revierte por completo: el `SupplierType` original se conserva, ningún documento queda eliminado y el usuario recibe un mensaje claro indicando que el cambio no se aplicó.
- ¿Qué ocurre con los periodos del año anterior que nunca tuvieron documento cargado al cruzar el 1 de enero? El sistema archiva esos periodos como "Faltante histórico" al inicio del año natural: siguen siendo visibles en el detalle del proveedor (colapsados bajo el año correspondiente) pero no reducen el indicador de cumplimiento activo del nuevo año. El recálculo se dispara automáticamente como parte del job diario (T093).
- ¿Qué pasa con un proveedor dado de alta el 1 de mayo 2026 respecto a los bimestres ene-feb y mar-abr de ese año? Esos periodos no se generan como "Faltante" porque `coverage_period_start < Supplier.created_at`. El primer requisito bimestral activo para ese proveedor es may-jun 2026.
- ¿Qué pasa si se sube un documento para un periodo anterior al alta del proveedor? El sistema lo acepta (no hay restricción de carga) pero ese documento no se cuenta como requisito activo ni aparece en el grid de cumplimiento principal; queda accesible en el historial del proveedor.
...
- **FR-005a**: Cada proveedor DEBE estar asociado a exactamente un **Tipo de Proveedor** del catálogo del tenant. Si el usuario no elige uno al crear, se asigna automáticamente el tipo "Sin clasificar" sembrado por el sistema (que tiene activos todos los tipos canónicos de documento). El usuario puede reasignar el tipo en cualquier momento desde la edición del proveedor; el cambio recalcula los documentos requeridos y el estado de cumplimiento agregado. Si el cambio implicaría eliminar documentos cuyo vencimiento cae en el año en curso, el flujo exige confirmación destructiva conforme a FR-005b, FR-005c y FR-005d.
- **FR-005b**: Al guardar un cambio de `SupplierType` sobre un proveedor existente, el sistema DEBE evaluar primero si existen documentos cargados de ese proveedor cuya `effective_due_date` cae dentro del **año natural en curso** (año actual del servidor en la zona horaria del tenant), considerando tanto la versión vigente como las versiones históricas archivadas. Si NO hay coincidencias, el cambio se aplica directamente y se recalculan los requisitos (FR-012b). Si hay al menos una coincidencia, el sistema DEBE bloquear la aplicación inmediata del cambio y solicitar la confirmación destructiva descrita en FR-005c antes de proceder.
- **FR-005c**: La confirmación destructiva del cambio de `SupplierType` DEBE presentarse como un popup modal que: (1) advierte que la acción eliminará permanentemente los documentos del proveedor cuyo vencimiento cae en el año en curso y muestra el conteo de documentos afectados (opcionalmente, una lista resumida); (2) contiene una caja de texto donde el usuario debe escribir exactamente la palabra `eliminar` (comparación case-insensitive, ignorando espacios al inicio/final); (3) mantiene el botón de confirmación deshabilitado mientras el texto introducido no coincida; (4) ofrece un botón de cancelar siempre habilitado que cierra el popup sin aplicar el cambio de tipo ni tocar los documentos.
- **FR-005d**: Al confirmar la acción destructiva de FR-005c, el sistema DEBE ejecutar en una sola operación transaccional: (a) eliminar de forma permanente todos los documentos del proveedor cuya `effective_due_date` cae en el año natural en curso, incluyendo sus archivos físicos almacenados y sus versiones históricas con vencimiento en ese mismo año; (b) aplicar el nuevo `SupplierType` al proveedor; (c) recalcular los documentos requeridos y el estado de cumplimiento agregado conforme a FR-012b y FR-014; (d) registrar en `audit_log` un evento por cada documento eliminado (acción `document.deleted_by_supplier_type_change`, con metadatos del tipo anterior y el tipo nuevo) y un evento `supplier.type_changed`. Si cualquiera de los pasos falla, la transacción DEBE revertirse completamente: el `SupplierType` original se conserva, los documentos no se eliminan y no quedan registros parciales.
...
- **FR-012b**: El conjunto de **documentos requeridos** para un proveedor se deriva exclusivamente de su **Tipo de Proveedor**: los `DocumentType` que la asociación `SupplierType ↔ DocumentType` activa para esa industria. Si la asociación define una `periodicity_override`, esa prevalece sobre la periodicidad base del `DocumentType` al calcular la fecha de vencimiento. El estado "Faltante" se evalúa SOLO contra esa lista (no contra todo el catálogo del tenant). La evaluación se aplica únicamente sobre los **periodos activos**, definidos como aquellos que cumplen simultáneamente: (a) `coverage_period_start ≤ today` (el periodo ya ha comenzado); y (b) `coverage_period_start ≥ Supplier.created_at` (el periodo no es anterior al alta del proveedor en el sistema). Los periodos futuros (`coverage_period_start > today`) NO generan obligación "Faltante". Los periodos del año anterior que nunca tuvieron documento cargado se archivan automáticamente al cruzar el año natural como **"Faltante histórico"**: visibles en el detalle del proveedor pero excluidos del indicador de cumplimiento activo (FR-014).
