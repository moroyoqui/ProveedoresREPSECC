...
- New for 009: Rol `supplier` en el User model + FK `users.supplier_id → suppliers.id`; SessionPayload extendido con `supplier_id`; nuevo módulo `repse/portal/` con 5 endpoints: `GET /compliance`, `GET /history/{id}`, `POST /upload`, `POST /submit/{id}`, `GET /submission/{id}`; nueva tabla `portal_submissions` para envíos a validación; frontend: PortalPage, redirección por rol, nav mínima, UploadPortalDialog, SubmitValidationButton, RejectionReasonBanner, selector de empresa en CreateUserDialog.
