...
def mysql_url() -> Iterator[str]:
    ...
    with MySqlContainer("mysql:8.0") as mysql:
        yield mysql.get_connection_url(driver="pymysql")
...
def db_session(engine) -> Iterator:  # type: ignore[no-untyped-def]
    ...
    conn = engine.connect()
    trans = conn.begin()
    session = Session(bind=conn, autoflush=False, autocommit=False, expire_on_commit=False)
    try:
        yield session
    finally:
        session.close()
...
@pytest.fixture()
def app(engine, monkeypatch):  # type: ignore[no-untyped-def]
